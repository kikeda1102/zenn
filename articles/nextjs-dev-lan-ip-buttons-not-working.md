...
Network のアドレスからアクセスすると localhost とは異なるオリジンとみなされ、HMR（Hot Module Replacement = コード変更をブラウザへ即座に反映する仕組み）用の WebSocket（サーバーとブラウザ間のリアルタイム通信）接続がブロックされる。

この WebSocket 接続が確立できないと、ハイドレーション（サーバーで生成した HTML に React のイベント処理を紐づける工程）が完了しない。
